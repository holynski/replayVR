...
for dataset in datasets:
    ...
    layer_args = ' --first_layer_image='+os.path.join(results_folder, 'min_composite_mrf.png') + \
           ...
           ' --first_layer_mesh='+os.path.join(dataset_path, 'textured.ply')+\
           ' --second_layer_mesh='+os.path.join(results_folder, 'second_layer_mesh.ply')+\
           ' --logtostderr'+\
           ' --v=2'

    if (os.system('./bin/layer_refiner' + layer_args) != 0):
        # shutil.rmtree(results_folder)
        quit()
