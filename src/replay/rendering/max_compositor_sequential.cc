...
MaxCompositorSequential::MaxCompositorSequential(
...
bool MaxCompositorSequential::Initialize() {
  std::string vertex_source;
  CHECK(OpenGLContext::ReadShaderFromFile(shader_src_dir + "/max_composite.vs",
                                          &vertex_source));
  std::string fragment_source;
  CHECK(OpenGLContext::ReadShaderFromFile(
      shader_src_dir + "/max_composite_seq.fs", &fragment_source));
...
bool MaxCompositorSequential::GetMaxComposite(
    const Reconstruction& reconstruction, const ImageCache& cache,
    const Camera& viewpoint, cv::Mat* output) {
  CHECK(is_initialized_) << "Initialize MaxCompositorSequential first.";
  ...
  *output = cv::Mat3b(image_size.y(), image_size.x(), cv::Vec3b(0,0,0));
