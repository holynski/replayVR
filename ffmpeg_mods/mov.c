...
static int64_t add_ctts_entry(MOVStts **ctts_data, unsigned int *ctts_count,
                              unsigned int *allocated_size, int count,
                              int duration);
...
static int mov_metadata_gnre(MOVContext *c, AVIOContext *pb, unsigned len,
                             const char *key) {
  short genre;
  ...
  genre = avio_r8(pb);
  if (genre < 1 || genre > ID3v1_GENRE_MAX) return 0;
  c->fc->event_flags |= AVFMT_EVENT_FLAG_METADATA_UPDATED;
...
static int mov_metadata_loci(MOVContext *c, AVIOContext *pb, unsigned len) {
  ...
  if (altitude) av_strlcatf(buf, sizeof(buf), "%+f", altitude);
...
retry:
      ...
      if (atom.type == MKTAG('c', 'o', 'v', 'r')) {
        ...
        if (index < c->meta_keys_count && index > 0) {
          ...
          av_log(c->fc, AV_LOG_WARNING,
                 "The index of 'data' is out of range: %" PRId32
                 " < 1 or >= %d.\n",
  ...
  if (!key) return 0;
  if (atom.size < 0 || str_size >= INT_MAX / 2) return AVERROR_INVALIDDATA;
  ...
  if (parse)
    ...
    if (!raw && (data_type == 3 ||
                 (data_type == 0 &&
                  (langcode < 0x400 || langcode == 0x7fff)))) {  // MAC Encoded
...
static int mov_read_chpl(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if ((atom.size -= 5) < 0) return 0;

  version = avio_r8(pb);
  avio_rb24(pb);
  if (version) avio_rb32(pb);  // ???
  nb_chapters = avio_r8(pb);

  for (i = 0; i < nb_chapters; i++) {
    if (atom.size < 9) return 0;

    start = avio_rb64(pb);
    str_len = avio_r8(pb);

    if ((atom.size -= 9 + str_len) < 0) return 0;

    ret = ffio_read_size(pb, str, str_len);
    if (ret < 0) return ret;
    str[str_len] = 0;
    avpriv_new_chapter(c->fc, i, (AVRational){1, 10000000}, start,
                       AV_NOPTS_VALUE, str);
  }
  return 0;
}
...
static int mov_read_dref(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!entries || entries > (atom.size - 1) / MIN_DATA_ENTRY_BOX_SIZE + 1 ||
  ...
  if (!sc->drefs) return AVERROR(ENOMEM);
  ...
  for (i = 0; i < entries; i++) {
    ...
    if (size < 12) return AVERROR_INVALIDDATA;
    ...
    if (dref->type == MKTAG('a', 'l', 'i', 's') && size > 150) {
      ...
      for (type = 0; type != -1 && avio_tell(pb) < next;) {
        ...
        if (type == 2) {  // absolute path
          ...
          if (!dref->path) return AVERROR(ENOMEM);
          ...
          for (j = 0; j < len; j++)
            if (dref->path[j] == ':' || dref->path[j] == 0) dref->path[j] = '/';
          av_log(c->fc, AV_LOG_DEBUG, "path %s\n", dref->path);
        } else if (type == 0) {  // directory name
          av_free(dref->dir);
          dref->dir = av_malloc(len + 1);
          if (!dref->dir) return AVERROR(ENOMEM);
      ...
      av_log(c->fc, AV_LOG_DEBUG,
             "Unknown dref type 0x%08" PRIx32 " size %" PRIu32 "\n", dref->type,
             size);
      entries--;
      i--;
    }
...
static int mov_read_hdlr(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (title_size > 0) {
    if (title_size > FFMIN(INT_MAX, SIZE_MAX - 1)) return AVERROR_INVALIDDATA;
    title_str = av_malloc(title_size + 1); /* Add null terminator */
    if (!title_str) return AVERROR(ENOMEM);
...
static int mov_read_dac3(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  ast = (enum AVAudioServiceType *)av_stream_new_side_data(
      st, AV_PKT_DATA_AUDIO_SERVICE_TYPE, sizeof(*ast));
  ...
  if (lfeon) st->codecpar->channel_layout |= AV_CH_LOW_FREQUENCY;
  *ast = bsmod;
...
static int mov_read_pasp(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if ((st->sample_aspect_ratio.den != 1 ||
       st->sample_aspect_ratio.num) &&  // default
      (den != st->sample_aspect_ratio.den ||
       num != st->sample_aspect_ratio.num)) {
    av_log(c->fc, AV_LOG_WARNING,
           "sample aspect ratio already set to %d:%d, ignoring 'pasp' atom "
           "(%d:%d)\n",
           st->sample_aspect_ratio.num, st->sample_aspect_ratio.den, num, den);
  } else if (den != 0) {
    av_reduce(&st->sample_aspect_ratio.num, &st->sample_aspect_ratio.den, num,
              den, 32767);
  }
  return 0;
}
...
static int mov_read_adrm(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  av_log(c->fc, AV_LOG_INFO,
         "[aax] file checksum == ");  // required by external tools
  for (i = 0; i < 20; i++) av_log(c->fc, AV_LOG_INFO, "%02x", file_checksum[i]);
  av_log(c->fc, AV_LOG_INFO, "\n");
  ...
  av_aes_crypt(c->aes_decrypt, output, input, DRM_BLOB_SIZE >> 4,
               intermediate_iv, 1);
  for (i = 0; i < 4; i++) {
...
static int mov_read_ftyp(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (comp_brand_size < 0) return AVERROR_INVALIDDATA;
  comp_brands_str = av_malloc(comp_brand_size + 1); /* Add null terminator */
  if (!comp_brands_str) return AVERROR(ENOMEM);
...
static int mov_read_moov(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if ((ret = mov_read_default(c, pb, atom)) < 0) return ret;
...
static MOVFragmentStreamInfo *get_frag_stream_info(MOVFragmentIndex *frag_index,
  ...
  if (index < 0 || index >= frag_index->nb_items) return NULL;
  item = &frag_index->item[index];
  for (i = 0; i < item->nb_stream_info; i++)
    if (item->stream_info[i].id == id) return &item->stream_info[i];
...
static void set_frag_stream(MOVFragmentIndex *frag_index, int id) {
  ...
  if (frag_index->current < 0 || frag_index->current >= frag_index->nb_items)
    return;
...
static int search_frag_moof_offset(MOVFragmentIndex *frag_index,
  ...
  while (b - a > 1) {
    ...
    if (moof_offset >= offset) b = m;
    if (moof_offset <= offset) a = m;
  }
  return b;
}
...
static int64_t get_frag_time(MOVFragmentIndex *frag_index, int index,
                             int track_id) {
  ...
  for (i = 0; i < frag_index->item[index].nb_stream_info; i++) {
    ...
    if (timestamp != AV_NOPTS_VALUE) return timestamp;
  }
  return AV_NOPTS_VALUE;
}

static int search_frag_timestamp(MOVFragmentIndex *frag_index, AVStream *st,
                                 int64_t timestamp) {
  int a, b, m;
  int64_t frag_time;
  int id = -1;
  ...
  while (b - a > 1) {
    ...
    if (frag_time != AV_NOPTS_VALUE) {
      if (frag_time >= timestamp) b = m;
      if (frag_time <= timestamp) a = m;
    }
  }
  return a;
}
...
static int update_frag_index(MOVContext *c, int64_t offset) {
  ...
  item = av_fast_realloc(
      c->frag_index.item, &c->frag_index.allocated_size,
      (c->frag_index.nb_items + 1) * sizeof(*c->frag_index.item));
  if (!item) return -1;
  c->frag_index.item = item;

  frag_stream_info =
      av_realloc_array(NULL, c->fc->nb_streams, sizeof(*item->stream_info));
  if (!frag_stream_info) return -1;
...
static int mov_read_colr(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  av_log(c->fc, AV_LOG_TRACE, "%s: pri %d trc %d matrix %d",
  ...
  if (!av_color_transfer_name(color_trc)) color_trc = AVCOL_TRC_UNSPECIFIED;
  if (!av_color_space_name(color_matrix)) color_matrix = AVCOL_SPC_UNSPECIFIED;
...
static int mov_read_fiel(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (c->fc->nb_streams < 1)  // will happen with jp2 files
    return 0;
  st = c->fc->streams[c->fc->nb_streams - 1];
  if (atom.size < 2) return AVERROR_INVALIDDATA;
...
static int mov_realloc_extradata(AVCodecParameters *par, MOVAtom atom) {
  int err = 0;
  uint64_t size = (uint64_t)par->extradata_size + atom.size + 8 +
...
/* Read a whole atom into the extradata return the size of the atom read,
 * possibly truncated if != atom.size */
static int64_t mov_read_atom_into_extradata(MOVContext *c, AVIOContext *pb,
                                            MOVAtom atom,
...
static int mov_read_extradata(MOVContext *c, AVIOContext *pb, MOVAtom atom,
  ...
  err = mov_read_atom_into_extradata(c, pb, atom, st->codecpar,
                                     st->codecpar->extradata + original_size);
  if (err < 0) return err;
  return 0;  // Note: this is the original behavior to ignore truncation.
}

/* wrapper functions for reading ALAC/AVS/MJPEG/MJPEG2000 extradata atoms only
 * for those codecs */
...
static int mov_read_avid(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (ret == 0) ret = mov_read_extradata(c, pb, atom, AV_CODEC_ID_DNXHD);
  return ret;
}
...
static int mov_read_ares(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  if (c->fc->nb_streams >= 1) {
    ...
    if (par->codec_tag == MKTAG('A', 'V', 'i', 'n') &&
        par->codec_id == AV_CODEC_ID_H264 && atom.size > 11) {
      int cid;
      avio_skip(pb, 10);
      cid = avio_rb16(pb);
      /* For AVID AVCI50, force width of 1440 to be able to select the correct
       * SPS and PPS */
      if (cid == 0xd4d || cid == 0xd4e) par->width = 1440;
      return 0;
...
static int mov_read_aclr(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (c->fc->nb_streams >= 1) {
    AVCodecParameters *par = c->fc->streams[c->fc->nb_streams - 1]->codecpar;
    if (par->codec_id == AV_CODEC_ID_H264) return 0;
    if (atom.size == 16) {
      original_size = par->extradata_size;
      ret = mov_realloc_extradata(par, atom);
      if (!ret) {
        length = mov_read_atom_into_extradata(c, pb, atom, par,
                                              par->extradata + original_size);
        if (length == atom.size) {
      ...
      av_log(c, AV_LOG_WARNING,
             "aclr not decoded - unexpected size %" PRId64 "\n", atom.size);
    }
  }
...
static int mov_read_wave(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if ((uint64_t)atom.size > (1 << 30)) return AVERROR_INVALIDDATA;
  ...
  if (st->codecpar->codec_id == AV_CODEC_ID_QDM2 ||
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_ALAC && atom.size >= 24) {
      ...
      if ((buffer & 0xFFFFFFFF) == MKBETAG('f', 'r', 'm', 'a') &&
          buffer >> 32 <= atom.size && buffer >> 32 >= 8) {
        avio_skip(pb, -8);
        atom.size += 8;
      } else if (!st->codecpar->extradata_size) {
#define ALAC_EXTRADATA_SIZE 36
        st->codecpar->extradata =
            av_mallocz(ALAC_EXTRADATA_SIZE + AV_INPUT_BUFFER_PADDING_SIZE);
...
static int mov_read_dvc1(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (atom.size >= (1 << 28) || atom.size < 7) return AVERROR_INVALIDDATA;

  profile_level = avio_r8(pb);
  if ((profile_level & 0xf0) != 0xc0) return 0;
...
static int mov_read_stco(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!sc->chunk_offsets) return AVERROR(ENOMEM);
...
static int mov_codec_id(AVStream *st, uint32_t format) {
  ...
  if (st->codecpar->codec_type != AVMEDIA_TYPE_VIDEO && id > 0) {
    ...
    if (id <= 0) id = ff_codec_get_id(ff_codec_bmp_tags, format);
    if (id > 0)
      st->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
      ...
      if (id > 0) st->codecpar->codec_type = AVMEDIA_TYPE_SUBTITLE;
    }
  }
...
static void mov_parse_stsd_video(MOVContext *c, AVIOContext *pb, AVStream *st,
                                 MOVStreamContext *sc) {
  ...
  if (len < 31) avio_skip(pb, 31 - len);

  if (codec_name[0]) av_dict_set(&st->metadata, "encoder", codec_name, 0);
...
static void mov_parse_stsd_audio(MOVContext *c, AVIOContext *pb, AVStream *st,
                                 MOVStreamContext *sc) {
  int bits_per_sample, flags;
  uint16_t version = avio_rb16(pb);
  AVDictionaryEntry *compatible_brands = av_dict_get(
      c->fc->metadata, "compatible_brands", NULL, AV_DICT_MATCH_CASE);
  ...
  if (!c->isom ||
    ...
    if (version == 1) {
      ...
      if (st->codecpar->codec_tag == MKTAG('l', 'p', 'c', 'm'))
        st->codecpar->codec_id = ff_mov_get_lpcm_codec_id(
            st->codecpar->bits_per_coded_sample, flags);
    }
  ...
  switch (st->codecpar->codec_id) {
    ...
    case AV_CODEC_ID_PCM_S16BE:
      if (st->codecpar->bits_per_coded_sample == 8)
        ...
        st->codecpar->codec_id = st->codecpar->codec_id == AV_CODEC_ID_PCM_S16BE
...
static uint32_t yuv_to_rgba(uint32_t ycbcr) {
  ...
  g = av_clip_uint8((1164 * (y - 16) - 813 * (cr - 128) - 391 * (cb - 128)) /
                    1000);
...
static int mov_rewrite_dvd_sub_extradata(AVStream *st) {
  ...
  st->codecpar->extradata =
      av_mallocz(strlen(buf) + AV_INPUT_BUFFER_PADDING_SIZE);
  if (!st->codecpar->extradata) return AVERROR(ENOMEM);
...
static int mov_parse_stsd_data(MOVContext *c, AVIOContext *pb, AVStream *st,
                               MOVStreamContext *sc, int64_t size) {
  int ret;

  if (st->codecpar->codec_tag == MKTAG('t', 'm', 'c', 'd')) {
    if ((int)size != size) return AVERROR(ENOMEM);
    ...
    if (size > 16) {
      ...
      st->avg_frame_rate.num =
          st->codecpar->extradata[16]; /* number of frame */
      ...
      if (size > 30) {
        uint32_t len =
            AV_RB32(st->codecpar->extradata + 18); /* name atom length */
        uint32_t format = AV_RB32(st->codecpar->extradata + 22);
        if (format == AV_RB32("name") && (int64_t)size >= (int64_t)len + 18) {
          uint16_t str_size =
              AV_RB16(st->codecpar->extradata + 26); /* string length */
          if (str_size > 0 && size >= (int)str_size + 26) {
            char *reel_name = av_malloc(str_size + 1);
            if (!reel_name) return AVERROR(ENOMEM);
...
static int mov_finalize_stsd_codec(MOVContext *c, AVIOContext *pb, AVStream *st,
                                   MOVStreamContext *sc) {
  ...
  switch (st->codecpar->codec_id) {
    ...
    case AV_CODEC_ID_QCELP:
      ...
      sc->samples_per_frame = 160;
      if (!sc->bytes_per_frame) sc->bytes_per_frame = 35;
      break;
...
static int mov_skip_multiple_stsd(MOVContext *c, AVIOContext *pb, int codec_tag,
                                  int format, int64_t size) {
...
int ff_mov_read_stsd_entries(MOVContext *c, AVIOContext *pb, int entries) {
  ...
  for (pseudo_stream_id = 0; pseudo_stream_id < entries && !pb->eof_reached;
       pseudo_stream_id++) {
    ...
    av_log(c->fc, AV_LOG_TRACE, "size=%" PRId64 " 4CC=%s codec_type=%d\n", size,
    ...
    if (a.size > 8) {
      if ((ret = mov_read_default(c, pb, a)) < 0) return ret;
    } else if (a.size > 0)
      avio_skip(pb, a.size);
    ...
    if (sc->extradata && st->codecpar->extradata) {
      ...
      sc->extradata[pseudo_stream_id] =
          av_malloc(extra_size + AV_INPUT_BUFFER_PADDING_SIZE);
      if (!sc->extradata[pseudo_stream_id]) return AVERROR(ENOMEM);
...
static int mov_read_stsd(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!sc->extradata) return AVERROR(ENOMEM);
  ...
  if (sc->extradata_size[0]) {
    st->codecpar->extradata =
        av_mallocz(sc->extradata_size[0] + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!st->codecpar->extradata) return AVERROR(ENOMEM);
...
static int mov_read_stsc(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  entries = avio_rb32(pb);
  if ((uint64_t)entries * 12 + 4 > atom.size) return AVERROR_INVALIDDATA;
  ...
  if (!sc->stsc_data) return AVERROR(ENOMEM);
...
static inline int mov_get_stsc_samples(MOVStreamContext *sc,
                                       unsigned int index) {
  int chunk_count;
...
static int mov_read_stps(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  entries = avio_rb32(pb);
  if (sc->stps_data) av_log(c->fc, AV_LOG_WARNING, "Duplicated STPS atom\n");
  av_free(sc->stps_data);
  sc->stps_count = 0;
  sc->stps_data = av_malloc_array(entries, sizeof(*sc->stps_data));
  if (!sc->stps_data) return AVERROR(ENOMEM);
...
static int mov_read_stss(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (entries >= UINT_MAX / sizeof(int)) return AVERROR_INVALIDDATA;
  av_freep(&sc->keyframes);
  sc->keyframe_count = 0;
  sc->keyframes = av_malloc_array(entries, sizeof(*sc->keyframes));
  if (!sc->keyframes) return AVERROR(ENOMEM);
...
static int mov_read_stsz(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!entries) return 0;
  if (entries >= (UINT_MAX - 4) / field_size) return AVERROR_INVALIDDATA;
  if (sc->sample_sizes) av_log(c->fc, AV_LOG_WARNING, "Duplicated STSZ atom\n");
  av_free(sc->sample_sizes);
  sc->sample_count = 0;
  sc->sample_sizes = av_malloc_array(entries, sizeof(*sc->sample_sizes));
  if (!sc->sample_sizes) return AVERROR(ENOMEM);
...
static int mov_read_stts(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (sc->stts_data) av_log(c->fc, AV_LOG_WARNING, "Duplicated STTS atom\n");
  av_freep(&sc->stts_data);
  sc->stts_count = 0;
  if (entries >= INT_MAX / sizeof(*sc->stts_data)) return AVERROR(ENOMEM);
  ...
  for (i = 0; i < entries && !pb->eof_reached; i++) {
    ...
    if (i + 1 == entries && i && sample_count == 1 &&
  ...
  if (duration) st->duration = duration;
...
static int mov_read_ctts(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!entries) return 0;
  if (entries >= UINT_MAX / sizeof(*sc->ctts_data)) return AVERROR_INVALIDDATA;
  av_freep(&sc->ctts_data);
  sc->ctts_data = av_fast_realloc(NULL, &sc->ctts_allocated_size,
                                  entries * sizeof(*sc->ctts_data));
  if (!sc->ctts_data) return AVERROR(ENOMEM);
  ...
  for (i = 0; i < entries && !pb->eof_reached; i++) {
    ...
    add_ctts_entry(&sc->ctts_data, &ctts_count, &sc->ctts_allocated_size, count,
                   duration);
    ...
    if (i + 2 < entries) mov_update_dts_shift(sc, duration);
  }
...
static int mov_read_sbgp(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (version == 1) avio_rb32(pb); /* grouping_type_parameter */
  ...
  if (!sc->rap_group) return AVERROR(ENOMEM);
 ...
 */
static int get_edit_list_entry(MOVContext *mov, const MOVStreamContext *msc,
  ...
  *edit_list_duration = msc->elst_data[edit_list_index].duration;
  ...
  if (global_timescale == 0) {
    avpriv_request_sample(mov->fc,
                          "Support for mvhd.timescale = 0 with editlists");
    return 0;
  }
  *edit_list_duration =
      av_rescale(*edit_list_duration, msc->time_scale, global_timescale);
  return 1;
}
 ...
 */
static int find_prev_closest_index(AVStream *st, AVIndexEntry *e_old,
                                   int nb_old, MOVStts *ctts_data,
                                   int64_t ctts_count, int64_t timestamp_pts,
                                   int flag, int64_t *index,
                                   int64_t *ctts_index, int64_t *ctts_sample) {
  ...
  *index =
      av_index_search_timestamp(st, timestamp_pts, flag | AVSEEK_FLAG_BACKWARD);
  ...
  if (*index >= 0) {
    for (i = *index; i > 0 && e_old[i].timestamp == e_old[i - 1].timestamp;
         i--) {
      if ((flag & AVSEEK_FLAG_ANY) || (e_old[i - 1].flags & AVINDEX_KEYFRAME)) {
        *index = i - 1;
      }
    }
  }
  ...
  if (ctts_data && *index >= 0) {
    ...
    *ctts_sample = 0;
    ...
    while (*index >= 0 && (*ctts_index) >= 0) {
      // Find a "key frame" with PTS <= timestamp_pts (So that we can decode
      // B-frames correctly).
      // No need to add dts_shift to the timestamp here becase timestamp_pts has
      // already been
      ...
      if (*ctts_sample == 0) {
        (*ctts_index)--;
        if (*ctts_index >= 0) *ctts_sample = ctts_data[*ctts_index].count - 1;
      } else {
        (*ctts_sample)--;
      }
    }
...
static int64_t add_index_entry(AVStream *st, int64_t pos, int64_t timestamp,
  ...
  entries = av_fast_realloc(st->index_entries,
                            &st->index_entries_allocated_size, requested_size);
  if (!entries) return -1;
...
static int64_t add_ctts_entry(MOVStts **ctts_data, unsigned int *ctts_count,
  ...
  if ((unsigned)(*ctts_count) + 1 >= UINT_MAX / sizeof(MOVStts)) return -1;

  ctts_buf_new = av_fast_realloc(*ctts_data, allocated_size, requested_size);

  if (!ctts_buf_new) return -1;
...
static void mov_estimate_video_delay(MOVContext *c, AVStream *st) {
  ...
  if (st->codecpar->video_delay <= 0 && msc->ctts_data &&
    ...
    for (ind = 0; ind < st->nb_index_entries && ctts_ind < msc->ctts_count;
      ...
      // Point j to the last elem of the buffer and insert the current pts
      // there.
      j = (buf_start + buf_size - 1) % buf_size;
      pts_buf[j] =
          st->index_entries[ind].timestamp + msc->ctts_data[ctts_ind].duration;

      // The timestamps that are already in the sorted buffer, and are greater
      // than the
      // current pts, are exactly the timestamps that need to be buffered to
      // output PTS
      // in correct sorted order.
      // Hence the video delay (which is the buffer size used to sort DTS and
      // output PTS),
      // can be computed as the maximum no. of swaps any particular timestamp
      // needs to
...
static void mov_current_sample_inc(MOVStreamContext *sc) {
  sc->current_sample++;
  sc->current_index++;
  if (sc->index_ranges && sc->current_index >= sc->current_index_range->end &&
...
static void mov_current_sample_dec(MOVStreamContext *sc) {
  sc->current_sample--;
  sc->current_index--;
  if (sc->index_ranges && sc->current_index < sc->current_index_range->start &&
...
static void mov_current_sample_set(MOVStreamContext *sc, int current_sample) {
  ...
  for (sc->current_index_range = sc->index_ranges; sc->current_index_range->end;
       sc->current_index_range++) {
...
static void mov_fix_index(MOVContext *mov, AVStream *st) {
  ...
  msc->index_ranges =
      av_malloc((msc->elst_count + 1) * sizeof(msc->index_ranges[0]));
  if (!msc->index_ranges) {
  ...
  while (get_edit_list_entry(mov, msc, edit_list_index, &edit_list_media_time,
    ...
    av_log(mov->fc, AV_LOG_DEBUG,
           "Processing st: %d, edit list %" PRId64 " - media time: %" PRId64
           ", duration: %" PRId64 "\n",
    ...
    // If we encounter a non-negative edit list reset the skip_samples/start_pad
    // fields and set them
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
      ...
      if (first_non_zero_audio_edit > 0) st->skip_samples = msc->start_pad = 0;
    }
    ...
    // We find closest previous key frame and preserve it and consequent frames
    // in index.
    // All frames which are outside edit list entry time boundaries will be
    // dropped after decoding.
    search_timestamp = edit_list_media_time;
    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
      // Audio decoders like AAC need need a decoder delay samples previous to
      // the current sample,
      // to correctly decode this frame. Hence for audio we seek to a frame 1
      // sec. before the
      // edit_list_media_time to cover the decoder delay.
      search_timestamp =
          FFMAX(search_timestamp - msc->time_scale, e_old[0].timestamp);
    }

    if (find_prev_closest_index(st, e_old, nb_old, ctts_data_old,
                                ctts_count_old, search_timestamp, 0, &index,
                                &ctts_index_old, &ctts_sample_old) < 0) {
      av_log(mov->fc, AV_LOG_WARNING,
             "st: %d edit list: %" PRId64
             " Missing key frame while searching for timestamp: %" PRId64 "\n",
             st->index, edit_list_index, search_timestamp);
      if (find_prev_closest_index(st, e_old, nb_old, ctts_data_old,
                                  ctts_count_old, search_timestamp,
                                  AVSEEK_FLAG_ANY, &index, &ctts_index_old,
                                  &ctts_sample_old) < 0) {
        av_log(mov->fc, AV_LOG_WARNING,
               "st: %d edit list %" PRId64
    ...
    for (; current < e_old_end; current++, index++) {
      ...
      if (ctts_data_old && ctts_index_old < ctts_count_old) {
        ...
        av_log(mov->fc, AV_LOG_DEBUG,
               "stts: %" PRId64 " ctts: %" PRId64 ", ctts_index: %" PRId64
               ", ctts_count: %" PRId64 "\n",
        ...
        if (ctts_sample_old == ctts_data_old[ctts_index_old].count) {
          if (add_ctts_entry(&msc->ctts_data, &msc->ctts_count,
            ...
            av_log(mov->fc, AV_LOG_ERROR,
                   "Cannot add CTTS entry %" PRId64 " - {%" PRId64 ", %d}\n",
                   ctts_index_old, ctts_data_old[ctts_index_old].count -
                                       edit_list_start_ctts_sample,
      ...
      if (curr_cts < edit_list_media_time ||
          curr_cts >= (edit_list_duration + edit_list_media_time)) {
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
          ...
          // Shift the index entry timestamp by packet_skip_samples to be
          // correct.
          edit_list_dts_counter -= packet_skip_samples;
          if (edit_list_start_encountered == 0) {
            edit_list_start_encountered = 1;
            // Make timestamps strictly monotonically increasing for audio, by
            // rewriting timestamps for
            // discarded packets.
            if (frame_duration_buffer) {
              fix_index_entry_timestamps(
                  st, st->nb_index_entries, edit_list_dts_counter,
          ...
          av_log(mov->fc, AV_LOG_DEBUG,
                 "drop a frame at curr_cts: %" PRId64 " @ %" PRId64 "\n",
                 curr_cts, index);

          if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
              edit_list_start_encountered == 0) {
            num_discarded_begin++;
            frame_duration_buffer = av_realloc(
                frame_duration_buffer, num_discarded_begin * sizeof(int64_t));
            if (!frame_duration_buffer) {
              av_log(mov->fc, AV_LOG_ERROR,
      ...
      if (add_index_entry(st, current->pos, edit_list_dts_counter,
                          current->size, current->min_distance, flags) == -1) {
        av_log(mov->fc, AV_LOG_ERROR, "Cannot add index entry\n");
        break;
      }

      // Update the index ranges array
      if (current_index_range < msc->index_ranges ||
          index != current_index_range->end) {
        current_index_range++;
      ...
      if ((curr_cts + frame_duration >=
        ...
        if (ctts_data_old) {
          ...
          // wait for one more, because there might be trailing B-frames after
          // this I-frame
  ...
  if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && min_corrected_pts > 0) {
    av_log(mov->fc, AV_LOG_DEBUG,
           "Offset DTS by %" PRId64 " to make first pts zero.\n",
           min_corrected_pts);
...
static void mov_build_index(MOVContext *mov, AVStream *st) {
  ...
  if (sc->elst_count) {
    ...
    if ((empty_duration || start_time) && mov->time_scale > 0) {
      if (empty_duration)
        empty_duration =
            av_rescale(empty_duration, sc->time_scale, mov->time_scale);
      sc->time_offset = start_time - empty_duration;
      if (!mov->advanced_editlist) current_dts = -sc->time_offset;
    }
  ...
  if (!(st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && sc->stts_count == 1 &&
        sc->stts_data[0].duration == 1)) {
    ...
    if (!sc->sample_count || st->nb_index_entries) return;
    if (sc->sample_count >=
        UINT_MAX / sizeof(*st->index_entries) - st->nb_index_entries)
      return;
    ...
    st->index_entries_allocated_size =
        (st->nb_index_entries + sc->sample_count) * sizeof(*st->index_entries);

    if (ctts_data_old) {
      // Expand ctts entries such that we have a 1-1 mapping with samples
      if (sc->sample_count >= UINT_MAX / sizeof(*sc->ctts_data)) return;
      sc->ctts_count = 0;
      sc->ctts_allocated_size = 0;
      sc->ctts_data =
          av_fast_realloc(NULL, &sc->ctts_allocated_size,
                          sc->sample_count * sizeof(*sc->ctts_data));
      if (!sc->ctts_data) {
        av_free(ctts_data_old);
        return;
      }
      for (i = 0; i < ctts_count_old && sc->ctts_count < sc->sample_count; i++)
        for (j = 0;
             j < ctts_data_old[i].count && sc->ctts_count < sc->sample_count;
             j++)
    ...
    for (i = 0; i < sc->chunk_count; i++) {
      int64_t next_offset =
          i + 1 < sc->chunk_count ? sc->chunk_offsets[i + 1] : INT64_MAX;
      current_offset = sc->chunk_offsets[i];
      while (mov_stsc_index_valid(stsc_index, sc->stsc_count) &&
             i + 1 == sc->stsc_data[stsc_index + 1].first)
        stsc_index++;

      if (next_offset > current_offset && sc->sample_size > 0 &&
          sc->sample_size < sc->stsz_sample_size &&
          sc->stsc_data[stsc_index].count * (int64_t)sc->stsz_sample_size >
      ...
      for (j = 0; j < sc->stsc_data[stsc_index].count; j++) {
        ...
        if (!sc->keyframe_absent &&
            (!sc->keyframe_count ||
             current_sample + key_off == sc->keyframes[stss_index])) {
          keyframe = 1;
          if (stss_index + 1 < sc->keyframe_count) stss_index++;
        } else if (sc->stps_count &&
                   current_sample + key_off == sc->stps_data[stps_index]) {
          keyframe = 1;
          if (stps_index + 1 < sc->stps_count) stps_index++;
        }
        if (rap_group_present && rap_group_index < sc->rap_group_count) {
          if (sc->rap_group[rap_group_index].index > 0) keyframe = 1;
        ...
        sample_size = sc->stsz_sample_size > 0
                          ? sc->stsz_sample_size
        ...
        if (sc->pseudo_stream_id == -1 ||
          ...
          av_log(mov->fc, AV_LOG_TRACE,
                 "AVIndex stream %d, sample %u, offset %" PRIx64
                 ", dts %" PRId64
                 ", "
                 "size %u, distance %u, keyframe %d\n",
                 st->index, current_sample, current_offset, current_dts,
                 sample_size, distance, keyframe);
          if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
              st->nb_index_entries < 100)
        ...
        if (sc->stts_data[stts_index].duration < 0) {
          av_log(mov->fc, AV_LOG_WARNING,
                 ...
                 sc->stts_data[stts_index].duration, stts_index, st->index);
        ...
        if (stts_index + 1 < sc->stts_count &&
            stts_sample == sc->stts_data[stts_index].count) {
          stts_sample = 0;
          stts_index++;
        }
      }
    ...
    for (i = 0; i < sc->stsc_count; i++) {
      ...
      if (i != sc->stsc_count - 1 && sc->samples_per_frame &&
      ...
      if (sc->samples_per_frame >= 160) {  // gsm
        ...
        unsigned samples =
            (1024 / sc->samples_per_frame) * sc->samples_per_frame;
    ...
    if (total >= UINT_MAX / sizeof(*st->index_entries) - st->nb_index_entries)
      return;
    if (av_reallocp_array(&st->index_entries, st->nb_index_entries + total,
                          sizeof(*st->index_entries)) < 0) {
      st->nb_index_entries = 0;
      return;
    }
    st->index_entries_allocated_size =
        (st->nb_index_entries + total) * sizeof(*st->index_entries);
      ...
      while (chunk_samples > 0) {
        ...
        if (sc->samples_per_frame > 1 && !sc->bytes_per_frame) {
          avpriv_request_sample(
              mov->fc, "Zero bytes per frame, but %d samples per frame",
        ...
        if (sc->samples_per_frame >= 160) {  // gsm
          ...
          if (sc->samples_per_frame > 1) {
            samples =
                FFMIN((1024 / sc->samples_per_frame) * sc->samples_per_frame,
                      chunk_samples);
        ...
        av_log(mov->fc, AV_LOG_TRACE,
               "AVIndex stream %d, chunk %u, offset %" PRIx64 ", dts %" PRId64
               ", "
               "size %u, duration %u\n",
...
static int test_same_origin(const char *src, const char *ref) {
  ...
  av_url_split(src_proto, sizeof(src_proto), src_auth, sizeof(src_auth),
               src_host, sizeof(src_host), &src_port, NULL, 0, src);
  av_url_split(ref_proto, sizeof(ref_proto), ref_auth, sizeof(ref_auth),
               ref_host, sizeof(ref_host), &ref_port, NULL, 0, ref);
  ...
  if (strlen(src) == 0) {
    ...
    return 0;
  } else if (strcmp(src_proto, ref_proto) || strcmp(src_auth, ref_auth) ||
             strcmp(src_host, ref_host) || src_port != ref_port) {
    return 0;
  } else
    return 1;
}
  ...
  if (ref->nlvl_to > 0 && ref->nlvl_from > 0) {
    ...
    if (i == ref->nlvl_to - 1 && src_path - src < sizeof(filename)) {
      ...
      if (!c->use_absolute_path) {
        ...
        if (!same_origin) {
          av_log(c->fc, AV_LOG_ERROR,
                 "Reference with mismatching origin, %s not tried for security "
                 "reasons, "
        ...
        if (strstr(ref->path + l + 1, "..") || strstr(ref->path + l + 1, ":") ||
            (ref->nlvl_from > 1 && same_origin < 0) ||
            (filename[0] == '/' && src_path == src))
          return AVERROR(ENOENT);
      }

      if (strlen(filename) + 1 == sizeof(filename)) return AVERROR(ENOENT);
      if (!c->fc->io_open(c->fc, pb, filename, AVIO_FLAG_READ, NULL)) return 0;
    }
  } else if (c->use_absolute_path) {
    av_log(c->fc, AV_LOG_WARNING,
           "Using absolute path on user request, "
           "this is a possible security issue\n");
    if (!c->fc->io_open(c->fc, pb, ref->path, AVIO_FLAG_READ, NULL)) return 0;
  } else {
    av_log(c->fc, AV_LOG_ERROR,
...
static void fix_timescale(MOVContext *c, MOVStreamContext *sc) {
  if (sc->time_scale <= 0) {
    ...
    if (sc->time_scale <= 0) sc->time_scale = 1;
  }
}
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
    ...
    if (c->enable_drefs) {
      if (mov_open_dref(c, &sc->pb, c->fc->url, dref) < 0)
        av_log(c->fc, AV_LOG_ERROR,
               ...
               st->index, dref->path, dref->dir, dref->filename, dref->volume,
  ...
  if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
    if (!st->sample_aspect_ratio.num && st->codecpar->width &&
        st->codecpar->height && sc->height && sc->width &&
        (st->codecpar->width != sc->width ||
         st->codecpar->height != sc->height)) {
      st->sample_aspect_ratio =
          av_d2q(((double)st->codecpar->height * sc->width) /
                     ((double)st->codecpar->width * sc->height),
                 INT_MAX);
    }

#if FF_API_R_FRAME_RATE
    if (sc->stts_count == 1 ||
        (sc->stts_count == 2 && sc->stts_data[1].count == 1))
      av_reduce(&st->r_frame_rate.num, &st->r_frame_rate.den, sc->time_scale,
                sc->stts_data[0].duration, INT_MAX);
#endif
  }
  ...
  // If the duration of the mp3 packets is not constant, then they could need a
  // parser
  if (st->codecpar->codec_id == AV_CODEC_ID_MP3 && sc->stts_count > 3 &&
...
static int mov_read_keys(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!c->meta_keys) return AVERROR(ENOMEM);

  for (i = 1; i <= count; ++i) {
    uint32_t key_size = avio_rb32(pb);
    uint32_t type = avio_rl32(pb);
    if (key_size < 8) {
      av_log(c->fc, AV_LOG_ERROR, "The key# %" PRIu32
    ...
    if (!c->meta_keys[i]) return AVERROR(ENOMEM);
...
static int mov_read_custom(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (end - avio_tell(pb) <= 12) break;

    len = avio_rb32(pb);
    tag = avio_rl32(pb);
    avio_skip(pb, 4);  // flags

    if (len < 12 || len - 12 > end - avio_tell(pb)) break;
    len -= 12;
  ...
  if (mean && key && val) {
    if (strcmp(key, "iTunSMPB") == 0) {
      ...
      if (sscanf(val, "%*X %X %X %X", &priming, &remainder, &samples) == 3) {
        if (priming > 0 && priming < 16384) sc->start_pad = priming;
      }
    }
...
#define IS_MATRIX_IDENT(matrix)                                         \
  ((matrix)[0][0] == (1 << 16) && (matrix)[1][1] == (1 << 16) &&        \
   (matrix)[2][2] == (1 << 30) && !(matrix)[0][1] && !(matrix)[0][2] && \
   !(matrix)[1][0] && !(matrix)[1][2] && !(matrix)[2][0] && !(matrix)[2][1])
...
static int mov_read_tkhd(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  flags = avio_rb24(pb);
  st->disposition |=
      (flags & MOV_TKHD_FLAG_ENABLED) ? AV_DISPOSITION_DEFAULT : 0;
    ...
    for (j = 0; j < 3; j++) {
      for (e = 0; e < 3; e++) {
        res_display_matrix[i][j] +=
            ((int64_t)display_matrix[i][e] * c->movie_display_matrix[e][j]) >>
            sh[e];
      }
    }
  }
  ...
  if (!IS_MATRIX_IDENT(res_display_matrix)) {
    ...
    if (!sc->display_matrix) return AVERROR(ENOMEM);
  ...
  if (width && height && sc->display_matrix) {
    ...
    for (i = 0; i < 2; i++)
      disp_transform[i] =
          hypot(sc->display_matrix[0 + i], sc->display_matrix[3 + i]);

    if (disp_transform[0] > 0 && disp_transform[1] > 0 &&
        disp_transform[0] < (1 << 24) && disp_transform[1] < (1 << 24) &&
        fabs((disp_transform[0] / disp_transform[1]) - 1.0) > 0.01)
      st->sample_aspect_ratio =
          av_d2q(disp_transform[0] / disp_transform[1], INT_MAX);
  }
  return 0;
}
...
static int mov_read_tfhd(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!track_id) return AVERROR_INVALIDDATA;
  ...
  frag->duration =
      flags & MOV_TFHD_DEFAULT_DURATION ? avio_rb32(pb) : trex->duration;
  frag->size = flags & MOV_TFHD_DEFAULT_SIZE ? avio_rb32(pb) : trex->size;
  frag->flags = flags & MOV_TFHD_DEFAULT_FLAGS ? avio_rb32(pb) : trex->flags;
...
static int mov_read_chap(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  num = atom.size / 4;
  if (!(new_tracks = av_malloc_array(num, sizeof(int)))) return AVERROR(ENOMEM);
...
static int mov_read_trex(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  c->fc->duration = AV_NOPTS_VALUE;  // the duration from mvhd is not
                                     // representing the whole file when
                                     // fragments are used.
...
static int mov_read_tfdt(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  sc = st->priv_data;
  if (sc->pseudo_stream_id + 1 != frag->stsd_id) return 0;
  version = avio_r8(pb);
  avio_rb24(pb); /* flags */
  if (version) {
  ...
  if (frag_stream_info) frag_stream_info->tfdt_dts = base_media_decode_time;
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (frag_stream_info) {
    if (frag_stream_info->first_tfra_pts != AV_NOPTS_VALUE &&
      ...
      av_log(c->fc, AV_LOG_DEBUG,
             "found mfra time %" PRId64 ", using it for pts\n", pts);
      ...
      av_log(c->fc, AV_LOG_DEBUG,
             "found sidx time %" PRId64 ", using it for pts\n", pts);
    } else if (frag_stream_info->tfdt_dts != AV_NOPTS_VALUE) {
      dts = frag_stream_info->tfdt_dts - sc->time_offset;
      av_log(c->fc, AV_LOG_DEBUG,
             "found tfdt time %" PRId64 ", using it for dts\n", dts);
    } else {
      dts = sc->track_end - sc->time_offset;
      av_log(c->fc, AV_LOG_DEBUG,
             "found track end time %" PRId64 ", using it for dts\n", dts);
    }
  } else {
    dts = sc->track_end - sc->time_offset;
    av_log(c->fc, AV_LOG_DEBUG,
  ...
  if ((unsigned)st->nb_index_entries + entries >=
      UINT_MAX / sizeof(AVIndexEntry)) {
  ...
  new_entries = av_fast_realloc(
      st->index_entries, &st->index_entries_allocated_size, requested_size);
  ...
  ctts_data =
      av_fast_realloc(sc->ctts_data, &sc->ctts_allocated_size, requested_size);
  if (!ctts_data) return AVERROR(ENOMEM);
  ...
  if (index_entry_pos < st->nb_index_entries) {
    ...
    memmove(
        st->index_entries + index_entry_pos + entries,
        st->index_entries + index_entry_pos,
        sizeof(*st->index_entries) * (st->nb_index_entries - index_entry_pos));
  ...
  if (frag_stream_info) frag_stream_info->index_entry = index_entry_pos;
  ...
  for (i = 0; i < entries && !pb->eof_reached; i++) {
    ...
    if (pts != AV_NOPTS_VALUE) {
      ...
      av_log(c->fc, AV_LOG_DEBUG,
             ...
             pts, dts, sc->dts_shift, ctts_duration, sc->time_offset,
    ...
    // decoding.
    if (prev_dts >= dts) index_entry_flags |= AVINDEX_DISCARD_FRAME;
    ...
    av_log(c->fc, AV_LOG_TRACE,
           "AVIndex stream %d, sample %d, offset %" PRIx64 ", dts %" PRId64
           ", "
           "size %u, distance %d, keyframe %d\n",
           st->index, index_entry_pos, offset, dts, sample_size, distance,
           keyframe);
    distance++;
    dts += sample_duration;
    offset += sample_size;
  ...
  if (i < entries) {
    ...
    memmove(
        sc->ctts_data + index_entry_pos, sc->ctts_data + index_entry_pos + gap,
        sizeof(*sc->ctts_data) * (sc->ctts_count - (index_entry_pos + gap)));
  ...
  for (i = index_entry_pos; i < st->nb_index_entries; i++) {
    if (prev_dts < st->index_entries[i].timestamp) break;
  ...
  fix_frag_index_entries(&c->frag_index, next_frag_index, frag->track_id,
                         entries);
  ...
  if (st->duration < sc->track_end) st->duration = sc->track_end;
...
static int mov_read_sidx(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  for (i = 0; i < item_count; i++) {
    ...
    if (frag_stream_info) frag_stream_info->sidx_pts = timestamp;
...
static int mov_read_cmov(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (avio_rl32(pb) != MKTAG('d', 'c', 'o', 'm')) return AVERROR_INVALIDDATA;
  if (avio_rl32(pb) != MKTAG('z', 'l', 'i', 'b')) {
    av_log(c->fc, AV_LOG_ERROR, "unknown compression for cmov atom !\n");
    return AVERROR_INVALIDDATA;
  }
  avio_rb32(pb); /* cmvd atom */
  if (avio_rl32(pb) != MKTAG('c', 'm', 'v', 'd')) return AVERROR_INVALIDDATA;
  moov_len = avio_rb32(pb); /* uncompressed size */
  cmov_len = atom.size - 6 * 4;

  cmov_data = av_malloc(cmov_len);
  if (!cmov_data) return AVERROR(ENOMEM);
  ...
  ret = AVERROR_INVALIDDATA;
  if (uncompress(moov_data, (uLongf *)&moov_len, (const Bytef *)cmov_data,
                 cmov_len) != Z_OK)
    goto free_and_return;
  if (ffio_init_context(&ctx, moov_data, moov_len, 0, NULL, NULL, NULL, NULL) !=
      0)
    goto free_and_return;
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (c->fc->nb_streams < 1 || c->ignore_editlist) return 0;
  ...
  if (atom.size != edit_count * elst_entry_size) {
    if (c->fc->strict_std_compliance >= FF_COMPLIANCE_STRICT) {
      ...
      if (edit_count * elst_entry_size != atom.size) {
        av_log(c->fc, AV_LOG_WARNING,
               "ELST atom of %" PRId64 " bytes, bigger than %d entries.",
               atom.size, edit_count);
      }
    }
  }

  if (!edit_count) return 0;
  ...
  if (!sc->elst_data) return AVERROR(ENOMEM);
  ...
  for (i = 0; i < edit_count && atom.size > 0 && !pb->eof_reached; i++) {
    ...
    atom.size -= 4;
    av_log(c->fc, AV_LOG_TRACE,
           "duration=%" PRId64 " time=%" PRId64 " rate=%f\n", e->duration,
           e->time, e->rate);
...
static int mov_read_tmcd(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (c->fc->nb_streams < 1) return AVERROR_INVALIDDATA;
  sc = c->fc->streams[c->fc->nb_streams - 1]->priv_data;
...
static int mov_read_vpcc(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  color_range =
      avio_r8(pb); /* bitDepth, chromaSubsampling, videoFullRangeFlag */
  ...
  if (!av_color_primaries_name(color_primaries))
    color_primaries = AVCOL_PRI_UNSPECIFIED;
  if (!av_color_transfer_name(color_trc)) color_trc = AVCOL_TRC_UNSPECIFIED;
  if (!av_color_space_name(color_space)) color_space = AVCOL_SPC_UNSPECIFIED;

  st->codecpar->color_range =
      (color_range & 1) ? AVCOL_RANGE_JPEG : AVCOL_RANGE_MPEG;
  st->codecpar->color_primaries = color_primaries;
  st->codecpar->color_trc = color_trc;
...
static int mov_read_smdm(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!sc->mastering) return AVERROR(ENOMEM);

  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++)
      sc->mastering->display_primaries[i][j] = av_make_q(
          lrint(((double)avio_rb16(pb) / (1 << 16)) * chroma_den), chroma_den);
  for (i = 0; i < 2; i++)
    sc->mastering->white_point[i] = av_make_q(
        lrint(((double)avio_rb16(pb) / (1 << 16)) * chroma_den), chroma_den);
  sc->mastering->max_luminance =
      av_make_q(lrint(((double)avio_rb32(pb) / (1 << 8)) * luma_den), luma_den);
  sc->mastering->min_luminance = av_make_q(
      lrint(((double)avio_rb32(pb) / (1 << 14)) * luma_den), luma_den);
...
static int mov_read_mdcv(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    sc->mastering->display_primaries[j][0] =
        av_make_q(avio_rb16(pb), chroma_den);
    sc->mastering->display_primaries[j][1] =
        av_make_q(avio_rb16(pb), chroma_den);
  }
...
static int mov_read_coll(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!sc->coll) return AVERROR(ENOMEM);
...
static int mov_read_st3d(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (!sc->stereo3d) return AVERROR(ENOMEM);
...
static int mov_read_sv3d(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  uint32_t mesh_crc, mesh_encoding, mesh_data_size;
  uint8_t *mesh;
  ...
  size = avio_rb32(pb);
  if (size <= 12 || size > atom.size) return AVERROR_INVALIDDATA;
  ...
  size = avio_rb32(pb);
  if (size > atom.size) return AVERROR_INVALIDDATA;
  ...
  switch (tag) {
    case MKTAG('c', 'b', 'm', 'p'):
      layout = avio_rb32(pb);
      if (layout) {
        av_log(c->fc, AV_LOG_WARNING, "Unsupported cubemap layout %d\n",
               layout);
        return 0;
      }
    ...
    case MKTAG('m', 's', 'h', 'p'):
      ...
      mesh_crc = avio_rb32(pb);
      mesh_encoding = avio_rb32(pb);

      if (mesh_encoding != 0x72617720 && mesh_encoding != 0x64666c38) {
        av_log(c->fc, AV_LOG_ERROR, "Encoding not recognized\n");
        return AVERROR_INVALIDDATA;
      }
      mesh_data_size = size - 8;
      if (mesh_data_size > atom.size) {
        av_log(c->fc, AV_LOG_ERROR, "Bad size!\n");
        return AVERROR_INVALIDDATA;
      }
      mesh = malloc(mesh_data_size);
      avio_read(pb, mesh, size);
      break;
    default:
  ...
  if (!sc->spherical) return AVERROR(ENOMEM);
  ...
  sc->spherical->mesh.data = mesh;
  sc->spherical->mesh.crc = mesh_crc;
  sc->spherical->mesh.encoding = mesh_encoding;
  sc->spherical->mesh.data_size = mesh_data_size;
...
static int mov_parse_uuid_spherical(MOVStreamContext *sc, AVIOContext *pb,
  ...
  if (!sc->spherical && av_stristr(buffer, "<GSpherical:StitchingSoftware>") &&
    ...
    if (val) sc->spherical->yaw = strtol(val, NULL, 10) * (1 << 16);
    val = av_stristr(buffer, "<GSpherical:InitialViewPitchDegrees>");
    if (val) sc->spherical->pitch = strtol(val, NULL, 10) * (1 << 16);
    val = av_stristr(buffer, "<GSpherical:InitialViewRollDegrees>");
    if (val) sc->spherical->roll = strtol(val, NULL, 10) * (1 << 16);
  }
...
static int mov_read_uuid(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  static const uint8_t uuid_xmp[] = {0xbe, 0x7a, 0xcf, 0xcb, 0x97, 0xa9,
  ...
  if (!memcmp(uuid, uuid_isml_manifest, sizeof(uuid))) {
    ...
    while ((ptr = av_stristr(ptr, "systemBitrate=\""))) {
      ...
      c->bitrates_count++;
      c->bitrates =
          av_realloc_f(c->bitrates, c->bitrates_count, sizeof(*c->bitrates));
      if (!c->bitrates) {
        c->bitrates_count = 0;
        av_free(buffer);
        return AVERROR(ENOMEM);
...
static int mov_read_senc(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (c->decryption_key_len == 0 || c->fc->nb_streams < 1) return 0;
...
static int mov_seek_auxiliary_info(MOVContext *c, MOVStreamContext *sc,
  ...
  if (sc->cenc.auxiliary_info_default_size) {
    ...
    if (index > sc->cenc.auxiliary_info_sizes_count) {
      av_log(c, AV_LOG_ERROR, "current sample %" PRId64
                              " greater than the number of auxiliary info "
                              "sample sizes %" SIZE_SPECIFIER "\n",
  ...
  if (auxiliary_info_seek_offset >
      sc->cenc.auxiliary_info_end - sc->cenc.auxiliary_info) {
    av_log(c, AV_LOG_ERROR,
           "auxiliary info offset %" SIZE_SPECIFIER
  ...
  sc->cenc.auxiliary_info_pos =
      sc->cenc.auxiliary_info + auxiliary_info_seek_offset;
  sc->cenc.auxiliary_info_index = index;
  return 0;
}

static int cenc_filter(MOVContext *c, MOVStreamContext *sc, int64_t index,
...
static int mov_read_dops(MOVContext *c, AVIOContext *pb, MOVAtom atom) {
  ...
  if (ff_alloc_extradata(st->codecpar, size)) return AVERROR(ENOMEM);
  ...
  st->codecpar->seek_preroll = av_rescale_q(
      OPUS_SEEK_PREROLL_MS, (AVRational){1, 1000}, (AVRational){1, 48000});
  ...
  while (total_size <= atom.size - 8 && !avio_feof(pb)) {
    ...
    if (atom.size >= 8) {
      ...
      if (a.type == MKTAG('f', 'r', 'e', 'e') && a.size >= 8 &&
        ...
        if (avio_read(pb, buf, 8) != 8) return AVERROR_INVALIDDATA;
        avio_seek(pb, -8, SEEK_CUR);
      ...
      total_size += 8;
      if (a.size == 1 &&
          total_size + 8 <= atom.size) { /* 64 bit extended size */
        a.size = avio_rb64(pb) - 8;
        total_size += 8;
      }
    }
    av_log(c->fc, AV_LOG_TRACE,
           "type:'%s' parent:'%s' sz: %" PRId64 " %" PRId64 " %" PRId64 "\n",
...
static int mov_probe(AVProbeData *p) {
  ...
  for (;;) {
    ...
    switch (tag) {
      ...
      case MKTAG(
          'p', 'n', 'o',
          't'): /* detect movs with preview pics like ew.mov and april.mov */
      case MKTAG(
          'u', 'd', 't',
          'a'): /* Packet Video PVAuthor adds this and a lot of more junk */
      case MKTAG('f', 't', 'y', 'p'):
  ...
  if (score > AVPROBE_SCORE_MAX - 50 && moov_offset != -1) {
    ...
    while (offset < (p->buf_size - 16)) { /* Sufficient space */
      ...
      if (AV_RL32(p->buf + offset) == MKTAG('h', 'd', 'l', 'r') &&
        ...
        av_log(NULL, AV_LOG_WARNING,
               "Found media data tag MPEG indicating this is a MOV-packed "
               "MPEG-PS.\n");
  ...
  for (j = 0; j < mov->nb_chapter_tracks; j++) {
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
      ...
      if (st->nb_index_entries) {
        ...
        if (av_get_packet(sc->pb, &pkt, sample->size) < 0) goto finish;
      ...
      for (i = 0; i < st->nb_index_entries; i++) {
        ...
        int64_t end = i + 1 < st->nb_index_entries
                          ? st->index_entries[i + 1].timestamp
                          : st->duration;
        uint8_t *title;
        uint16_t ch;
        int len, title_len;
        ...
        len = avio_rb16(sc->pb);
        if (len > sample->size - 2) continue;
        title_len = 2 * len + 1;
        if (!(title = av_mallocz(title_len))) goto finish;

        // The samples could theoretically be in any encoding if there's an encd
        // atom following, but in practice are only utf-8 or utf-16,
        // distinguished
...
static int mov_read_rtmd_track(AVFormatContext *s, AVStream *st) {
  ...
  if (!st->nb_index_entries) return -1;
  ...
  drop = avio_r8(s->pb);
  ff = avio_r8(s->pb);
  snprintf(buf, AV_TIMECODE_STR_SIZE, "%02d:%02d:%02d%c%02d", hh, mm, ss,
           drop ? ';' : ':', ff);
...
static int mov_read_close(AVFormatContext *s) {
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    if (!sc->pb_is_copied) ff_format_io_close(s, &sc->pb);
    ...
    if (sc->extradata)
      for (j = 0; j < sc->stsd_count; j++) av_free(sc->extradata[j]);
    av_freep(&sc->extradata);
    av_freep(&sc->extradata_size);
...
/* look for a tmcd track not referenced by any video track, and export it
 * globally */
...
static int read_tfra(MOVContext *mov, AVIOContext *f) {
  ...
  for (i = 0; i < item_count; i++) {
    ...
    if (frag_stream_info && frag_stream_info->first_tfra_pts == AV_NOPTS_VALUE)
      frag_stream_info->first_tfra_pts = time;

    for (j = 0; j < ((fieldlength >> 4) & 3) + 1; j++) avio_r8(f);
    for (j = 0; j < ((fieldlength >> 2) & 3) + 1; j++) avio_r8(f);
    for (j = 0; j < ((fieldlength >> 0) & 3) + 1; j++) avio_r8(f);
  }
...
fail:
  ...
  if (seek_ret < 0) {
    av_log(c->fc, AV_LOG_ERROR, "failed to seek back after looking for mfra\n");
    ret = seek_ret;
  }
  return ret;
}
...
static int mov_read_header(AVFormatContext *s) {
  ...
  mov->fc = s;
  mov->trak_index = -1;
  /* .mov and .mp4 aren't streamable anyway (only progressive download if moov
   * is before mdat) */
  if (pb->seekable & AVIO_SEEKABLE_NORMAL)
  ...
  /* check MOV header */
  do {
    if (mov->moov_retry) avio_seek(pb, 0, SEEK_SET);
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    if (sc->timecode_track > 0) {
      ...
      for (j = 0; j < s->nb_streams; j++)
        if (s->streams[j]->id == sc->timecode_track) tmcd_st_id = j;

      if (tmcd_st_id < 0 || tmcd_st_id == i) continue;
      tcr = av_dict_get(s->streams[tmcd_st_id]->metadata, "timecode", NULL, 0);
      if (tcr) av_dict_set(&st->metadata, "timecode", tcr->value, 0);
    }
  }
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
        sc->nb_frames_for_fps > 0 && sc->duration_for_fps > 0)
      av_reduce(&st->avg_frame_rate.num, &st->avg_frame_rate.den,
                sc->time_scale * (int64_t)sc->nb_frames_for_fps,
                sc->duration_for_fps, INT_MAX);
    if (st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) {
      ...
      if (st->codecpar->codec_id == AV_CODEC_ID_DVD_SUBTITLE) {
        if ((err = mov_rewrite_dvd_sub_extradata(st)) < 0) return err;
      }
    }
      ...
      if (st->duration > 0) {
        if (sc->data_size > INT64_MAX / sc->time_scale / 8) {
          av_log(s, AV_LOG_ERROR,
                 "Overflow during bit rate calculation %" PRId64 " * 8 * %d\n",
  ...
  if (mov->use_mfra_for > 0) {
    for (i = 0; i < s->nb_streams; i++) {
      ...
      if (sc->duration_for_fps > 0) {
        ...
        st->codecpar->bit_rate =
            sc->data_size * 8 * sc->time_scale / sc->duration_for_fps;
      }
    }
  }
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    switch (st->codecpar->codec_type) {
      ...
      case AVMEDIA_TYPE_VIDEO:
        ...
        if (sc->mastering) {
          err = av_stream_add_side_data(
              st, AV_PKT_DATA_MASTERING_DISPLAY_METADATA,
              (uint8_t *)sc->mastering, sizeof(*sc->mastering));
          if (err < 0) return err;
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st) {
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    if (msc->pb && msc->current_sample < avst->nb_index_entries) {
      ...
      int64_t dts =
          av_rescale(current_sample->timestamp, AV_TIME_BASE, msc->time_scale);
...
static int should_retry(AVIOContext *pb, int error_code) {
  if (error_code == AVERROR_EOF || avio_feof(pb)) return 0;
...
static int mov_switch_root(AVFormatContext *s, int64_t target, int index) {
  ...
  if (index < mov->frag_index.nb_items) {
    ...
    if (mov->frag_index.item[index].headers_read) return 0;
...
static int mov_change_extradata(MOVStreamContext *sc, AVPacket *pkt) {
  ...
  if (extradata_size > 0 && extradata) {
    side =
        av_packet_new_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, extradata_size);
...
retry:
  ...
  if (!sample || (mov->next_root_atom && sample->pos > mov->next_root_atom)) {
    if (!mov->next_root_atom) return AVERROR_EOF;
    if ((ret = mov_switch_root(s, mov->next_root_atom, -1)) < 0) return ret;
    goto retry;
  }
  sc = st->priv_data;
  ...
  if (st->discard != AVDISCARD_ALL) {
    ...
    if (st->discard == AVDISCARD_NONKEY &&
        0 == (sample->flags & AVINDEX_KEYFRAME)) {
      av_log(mov->fc, AV_LOG_DEBUG,
  ...
  if (sc->ctts_data && sc->ctts_index < sc->ctts_count) {
    pkt->pts =
        pkt->dts + sc->dts_shift + sc->ctts_data[sc->ctts_index].duration;
    /* update ctts context */
    sc->ctts_sample++;
  ...
  if (mov->aax_mode) aax_filter(pkt->data, pkt->size, mov);
...
static int mov_seek_fragment(AVFormatContext *s, AVStream *st,
                             int64_t timestamp) {
  MOVContext *mov = s->priv_data;
  int index;

  if (!mov->frag_index.complete) return 0;
...
static int mov_read_seek(AVFormatContext *s, int stream_index,
  ...
  if (stream_index >= s->nb_streams) return AVERROR_INVALIDDATA;
  ...
  if (mc->seek_individually) {
    ...
    for (i = 0; i < s->nb_streams; i++) {
      ...
      if (stream_index == i) continue;

      timestamp = av_rescale_q(
          seek_timestamp, s->streams[stream_index]->time_base, st->time_base);
    ...
    while (1) {
      ...
      if (!entry) return AVERROR_INVALIDDATA;
      sc = st->priv_data;
      if (sc->ffindex == stream_index && sc->current_sample == sample) break;
      mov_current_sample_inc(sc);
    }
  }
  return 0;
...
static const AVOption mov_options[] = {
    ...
    {"ignore_editlist",
     ...
     FLAGS},
    {"advanced_editlist",
     "Modify the AVIndex according to the editlists. Use this option to decode "
     "in the order specified by the edits.",
     OFFSET(advanced_editlist),
     AV_OPT_TYPE_BOOL,
     {.i64 = 1},
     0,
     1,
    ...
    {"activation_bytes", "Secret bytes for Audible AAX files",
     OFFSET(activation_bytes), AV_OPT_TYPE_BINARY,
    ...
    {"audible_fixed_key",  // extracted from libAAX_SDK.so and AAXSDKWin.dll
     ...
     .flags = AV_OPT_FLAG_DECODING_PARAM},
    {"decryption_key", "The media decryption key (hex)", OFFSET(decryption_key),
