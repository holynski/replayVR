...
void writeTextImage(const char* filename, uint width, uint height,
                    float* image) {
...
void writeGradients(char* file, int width, int height, int pitchInFloats,
                    int norients, int scales, float* pb) {
  int fd;
  ...
  for (int scale = 0; scale < scales; scale++) {
    for (int orient = 0; orient < norients; orient++) {
      float* currentPointer =
          &pb[pitchInFloats * orient + pitchInFloats * scale * norients];
...
void checkInputValue(int& nEigNum, float& fEigTolerance, int& nTextonChoice) {
  ...
  if (nTextonChoice > 2 || nTextonChoice < 1) {
    printf(
        "\nException: Only support choice 1 (32 bins) and choice 2 (64 "
        "bins)\n");
  }
}

void parsingCommand(int argc, char** argv, int& nEigNum, float& fEigTolerance,
                    int& nTextonChoice) {
  if (argc < 3) {
    nEigNum = 9;
    fEigTolerance = 1e-3;
...
int main_single_patch(uint width, uint height, uint* devRgbU, float** pdevGPb,
                      float** pdevGPball, float** pdevMPb,
                      int& matrixPitchInFloats, int nTextonChoice, int nEigNum,
                      float fEigTolerance) {
  ...
  localCues(width, height, devL, devA, devB, devTextons, &devBg, &devCga,
            &devCgb, &devTg, &matrixPitchInFloats, nTextonChoice);
  ...
  combine(width, height, matrixPitchInFloats, devBg, devCga, devCgb, devTg,
          &devMPbO, &devCombinedGradient, nTextonChoice);
  ...
  *pdevMPb = devMPb;
  ...
  // int nEigNum = 17;
  generalizedEigensolve(theStencil, devMatrix, matrixPitchInFloats, nEigNum,
                        &eigenvalues, &devEigenvectors, fEigTolerance);
  ...
  CUDA_SAFE_CALL(cudaMallocPitch((void**)&devSPb, &devSPb_pitch,
                                 nPixels * sizeof(float), 8));
  cudaMemset(devSPb, 0, matrixPitchInFloats * sizeof(float) * 8);

  spectralPb(eigenvalues, devEigenvectors, width, height, nEigNum, devSPb,
             matrixPitchInFloats);
  ...
  CUDA_SAFE_CALL(
      cudaMalloc((void**)&devGPball, sizeof(float) * matrixPitchInFloats * 8));
  StartCalcGPb(nPixels, matrixPitchInFloats, 8, devCombinedGradient, devSPb,
               devMPb, devGPball, devGPb);
  *pdevGPb = devGPb;
...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    printf(
        "\nUsage: %s input_image.ppm eigenvector_num eigenvector_tolerance "
        "texton_choice",
        argv[0]);
  ...
  //--------------------------------------------------------------------------------------------
  // decompose the image into multiple parts if it is too big, make sure there
  // is enough padding
  {
    ...
    if ((width <= max_patch_length) && (height <= max_patch_length)) {
      ...
      main_single_patch(width, height, devRgbU, &tpatchDevGPb, &tpatchDevGPball,
                        &tpatchDevMPb, tpatchMatrixPitchInFloats, nTextonChoice,
                        nEigNum, fEigTolerance);
      ...
      int w_seg =
          width / max_patch_length + ((width % max_patch_length) == 0 ? 0 : 1);
      int h_seg = height / max_patch_length +
                  ((height % max_patch_length) == 0 ? 0 : 1);
      ...
      CUDA_SAFE_CALL(cudaMalloc((void**)&devGPb, sizeof(float) * nPixels));
      CUDA_SAFE_CALL(cudaMalloc((void**)&devGPball,
                                sizeof(float) * matrixPitchInFloats * 8));
      CUDA_SAFE_CALL(cudaMalloc((void**)&devMPb, sizeof(float) * nPixels));

      for (int ih = 0; ih < h_seg; ++ih) {
        for (int iw = 0; iw < w_seg; ++iw) {
          fprintf(stdout,
                  "\nimage_size = [%d, %d], ih, iw = [%d, %d], seg_size = [%d, "
                  "%d]\n",
          ...
          fprintf(stdout, "\nXY start = [%d, %d], top left pad = [%d, %d]\n",
                  xstart, ystart, top_pad, left_pad);
          ...
          for (int ik = 0; ik < patch_height; ++ik) {
            ...
            uint* psource =
                &(devRgbU[(ik + patch_ystart) * width + patch_xstart]);
            cudaMemcpy(ptarget, psource, patchLineSizeUint,
                       cudaMemcpyDeviceToDevice);
          }
          main_single_patch(patch_width, patch_height, tpatchDevRgbU,
                            &tpatchDevGPb, &tpatchDevGPball, &tpatchDevMPb,
                            tpatchMatrixPitchInFloats, nTextonChoice, nEigNum,
                            fEigTolerance);
          // copy data out
          ...
          for (int ik = 0; ik < seg_height; ++ik) {
            ...
            float* psource =
                &(tpatchDevGPb[(ik + top_pad) * patch_width + left_pad]);
            cudaMemcpy(ptarget, psource, patchLineInnerSizeFloat,
                       cudaMemcpyDeviceToDevice);
          }
          for (int jj = 0; jj < 8; ++jj) {
            for (int ik = 0; ik < seg_height; ++ik) {
              float* ptarget = &(devGPball[jj * matrixPitchInFloats +
                                           (ik + ystart) * width + xstart]);
              float* psource =
                  &(tpatchDevGPball[jj * tpatchMatrixPitchInFloats +
                                    (ik + top_pad) * patch_width + left_pad]);
              cudaMemcpy(ptarget, psource, patchLineInnerSizeFloat,
                         cudaMemcpyDeviceToDevice);
            }
          }
          for (int ik = 0; ik < seg_height; ++ik) {
            ...
            float* psource =
                &(tpatchDevMPb[(ik + top_pad) * patch_width + left_pad]);
            cudaMemcpy(ptarget, psource, patchLineInnerSizeFloat,
                       cudaMemcpyDeviceToDevice);
          }
          // fprintf(stdout, "%p, %p, %p, %p\n", tpatchDevRgbU, tpatchDevGPb,
          // tpatchDevGPball, tpatchDevMPb);
          // CUDA_SAFE_CALL(cudaFree(tpatchDevRgbU));		// already been
          // released
...
//--------------------------------------------------------------------------------------------
  ...
  PostProcess(
      width, height, width, devGPb, devMPb,
      devGPb_thin);  // note: 3rd param width is the actual pitch of the image
  ...
  cudaMemcpy(hostGPb_thin, devGPb_thin, sizeof(float) * nPixels,
  ...
  cudaMemcpy(hostGPbAll, devGPball, sizeof(float) * matrixPitchInFloats * 8,
             cudaMemcpyDeviceToHost);

  // csweeney tested the outputs of each orientation on a line-star image to
  // determine which directions are which (since they aren't labelled anywhere
  // in the code!!!). It seems that 0 is vertical and 4 is horizontal.


  // int oriMap[] = {3, 2, 1, 0, 7, 6, 5, 4};
  // float* hostGPbAllConcat = (float*)malloc(sizeof(float) * width * height * 8);
  // for (int i = 0; i < 8; i++) {
  //   transpose(width, height, hostGPbAll + matrixPitchInFloats * oriMap[i],
  //             hostGPbAllConcat + width * height * i);
  // }
  // int dim[3];
  // dim[0] = 8;
  // dim[1] = width;
  ...
  // For our purposes, only output the horizontal and vertical responses
  int oriMap[] = {3, 0};
  float* hostGPbAllConcat = (float*)malloc(sizeof(float) * width * height * 2);
  for (int i = 0; i < 2; i++) {
    transpose(width, height, hostGPbAll + matrixPitchInFloats * oriMap[i],
              hostGPbAllConcat + width * height * i);
  }
