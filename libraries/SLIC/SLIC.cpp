...
//=================================================================================
void SLIC::DrawContoursAroundSegments(unsigned int *ubuff, const int *labels,
    ...
    for (int k = 0; k < width; k++) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if ((x >= 0 && x < width) && (y >= 0 && y < height)) {
          ...
          if (false ==
              istaken[index]) // comment this to obtain internal contours
          {
...
/// statement inside the loop that looks at neighbourhood.
//=================================================================================
void SLIC::DrawContoursAroundSegmentsTwoColors(unsigned int *img,
                                               const int *labels,
                                               const int &width,
                                               const int &height) {
    ...
    for (int k = 0; k < width; k++) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if ((x >= 0 && x < width) && (y >= 0 && y < height)) {
          ...
          // if( false == istaken[index] )//comment this to obtain internal
          // contours
          {
            if (labels[mainindex] != labels[index])
              np++;
...
void SLIC::DetectLabEdges(const double *lvec, const double *avec,
                          ...
                          const int &height, std::vector<double> &edges) {
  int sz = width * height;
...
///	PerturbSeeds
//===========================================================================
void SLIC::PerturbSeeds(std::vector<double> &kseedsl,
...
void SLIC::GetLABXYSeeds_ForGivenStepSize(
    ...
    std::vector<double> &kseedsy, const int &STEP, const bool &perturbseeds,
...
/// The k seed values are taken as uniform spatial pixel samples.
//===========================================================================
void SLIC::GetLABXYSeeds_ForGivenK(std::vector<double> &kseedsl,
                                   std::vector<double> &kseedsa,
                                   std::vector<double> &kseedsb,
                                   std::vector<double> &kseedsx,
                                   std::vector<double> &kseedsy, const int &K,
...
void SLIC::PerformSuperpixelSegmentation_VariableSandM(
  ...
  std::vector<double> distlab(sz, DBL_MAX);
  std::vector<double> distvec(sz, DBL_MAX);
  std::vector<double> maxlab(
      numk, 10 * 10); // THIS IS THE VARIABLE VALUE OF M, just start with 10
  std::vector<double> maxxy(
      numk, STEP * STEP); // THIS IS THE VARIABLE VALUE OF M, just start with 10
  ...
  while (numitr < NUMITR) {
    ...
    for (int n = 0; n < numk; n++) {
      int y1 = std::max<int>(0, kseedsy[n] - offset);
      int y2 = std::min<int>(m_height, kseedsy[n] + offset);
      int x1 = std::max<int>(0, kseedsx[n] - offset);
      int x2 = std::min<int>(m_width, kseedsx[n] + offset);
      ...
      for (int y = y1; y < y2; y++) {
        for (int x = x1; x < x2; x++) {
          ...
          double dist =
              distlab[i] / maxlab[n] +
              distxy[i] * invxywt; // only varying m, prettier superpixels
          // double dist = distlab[i]/maxlab[n] + distxy[i]/maxxy[n];//varying
          // both m and S
          //------------------------------------------------------------------------
...

    {
      for (int k = 0; k < numk; k++) {
        ...
        if (clustersize[k] <= 0)
          clustersize[k] = 1;
        inv[k] = 1.0 / double(clustersize[k]); // computing inverse now to
...
// 	const string&				path)
// {
//   static const int kMaxFname = 256;
//   int sz = width*height;

// 	char fname[kMaxFname];
// 	char extn[kMaxFname];
...
///found
///		    adjacent label to this component, and not incrementing the
///label.
//===========================================================================
...
//===========================================================================
void SLIC::PerformSLICO_ForGivenStepSize(const unsigned int *ubuff,
                                         const int width, const int height,
  ...
  EnforceLabelConnectivity(klabels, m_width, m_height, nlabels, numlabels,
