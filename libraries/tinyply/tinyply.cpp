...
PlyElement::PlyElement(std::istream &is) { parse_internal(is); }

void PlyElement::parse_internal(std::istream &is) { is >> name >> size; }
...
bool PlyFile::parse_header(std::istream &is) {
  ...
  while (std::getline(is, line)) {
    ...
    if (token == "ply" || token == "PLY" || token == "") {
      ...
      read_header_text(line, ls, objInfo, 8);
...
void PlyFile::read_header_text(std::string line, std::istream &is,
                               std::vector<std::string> &place, int erase) {
...
uint32_t PlyFile::skip_property_binary(const PlyProperty &property,
                                       std::istream &is) {
...
void PlyFile::skip_property_ascii(const PlyProperty &property,
                                  std::istream &is) {
  std::string skip;
  if (property.isList) {
    int listSize;
...
void PlyFile::read_property_binary(PlyProperty::Type t, void *dest,
                                   uint32_t &destOffset, std::istream &is) {
  ...
  switch (t) {
    ...
    case PlyProperty::Type::FLOAT32:
      ply_cast<float>(dest, src.data());
      break;
    case PlyProperty::Type::FLOAT64:
      ply_cast<double>(dest, src.data());
      break;
...
void PlyFile::read_property_ascii(PlyProperty::Type t, void *dest,
                                  uint32_t &destOffset, std::istream &is) {
  switch (t) {
    case PlyProperty::Type::INT8:
...
void PlyFile::write_property_ascii(PlyProperty::Type t, std::ostringstream &os,
                                   uint8_t *src, uint32_t &srcOffset) {
  switch (t) {
    case PlyProperty::Type::INT8:
...
void PlyFile::write_property_binary(PlyProperty::Type t, std::ostringstream &os,
                                    uint8_t *src, uint32_t &srcOffset) {
  os.write(reinterpret_cast<const char *>(src), PropertyTable[t].stride);
  srcOffset += PropertyTable[t].stride;
}

void PlyFile::read(std::istream &is) { read_internal(is); }
    ...
    for (int i = 0; i < e.size; ++i) {
      for (auto &p : e.properties) {
        ...
        if (p.isList) {
          ...
          write_property_ascii(p.propertyType, os,
                               (cursor->data + cursor->offset), cursor->offset);
        }
      }
      os << std::endl;
    }
...
void PlyFile::write_header(std::ostringstream &os) {
  ...
  for (const auto &comment : comments) os << "comment " << comment << std::endl;
...
void PlyFile::read_internal(std::istream &is) {
  std::function<void(PlyProperty::Type t, void *dest, uint32_t &destOffset,
                     std::istream &is)> read;
  std::function<void(const PlyProperty &property, std::istream &is)> skip;
  if (isBinary) {
    read = [&](PlyProperty::Type t, void *dest, uint32_t &destOffset,
               std::istream &is) {
    ...
    read =
        [&](PlyProperty::Type t, void *dest, uint32_t &destOffset,
            std::istream &is) { read_property_ascii(t, dest, destOffset, is); };
    skip = [&](const PlyProperty &property, std::istream &is) {
          ...
          if (auto &cursor =
            ...
            if (property.isList) {
              ...
              if (cursor->realloc == false) {
                cursor->realloc = true;
                resize_vector(property.propertyType, cursor->vector,
                              listSize * element.size, cursor->data);
              }
              for (auto i = 0; i < listSize; ++i) {
                read(property.propertyType, (cursor->data + cursor->offset),
                     cursor->offset, is);
              }
            } else {
              read(property.propertyType, (cursor->data + cursor->offset),
