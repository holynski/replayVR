...
#ifndef TINYPLY_TINYPLY_H_
#define TINYPLY_TINYPLY_H_
...
class PlyProperty {
 public:
  ...
  PlyProperty(Type list_type, Type prop_type, int listCount,
...
inline void resize_vector(const PlyProperty::Type t, void *v, int32_t newSize,
                          uint8_t *&ptr) {
  switch (t) {
...
class PlyFile {
 public:
  ...
  template <typename T>
  int request_properties_from_element(std::string elementKey,
    ...
    for (auto key : propertyKeys) {
      if (int instanceCount = instance_counter(elementKey, key)) {
        ...
        if (result.second == false)
          throw std::runtime_error("property has already been requested: " +
                                   key);
      } else
        return 0;
    }

    uint32_t totalInstanceSize = [&]() {
      uint32_t t = 0;
  ...
  void add_properties_to_element(
      std::string elementKey, std::vector<std::string> propertyKeys,
      std::vector<T> &source, int listCount = 1,
      PlyProperty::Type listType = PlyProperty::Type::INVALID) {
    auto cursor = std::make_shared<DataCursor>();
    cursor->offset = 0;
    cursor->vector = &source;
    cursor->data = reinterpret_cast<uint8_t *>(source.data());
    ...
    auto create_property_on_element = [&](PlyElement &e) {
      for (auto key : propertyKeys) {
        ...
        PlyProperty newProp = (listType == PlyProperty::Type::INVALID)
                                  ? PlyProperty(t, key)
                                  : PlyProperty(listType, t, listCount, key);
      ...
      PlyElement newElement =
          listCount == 1
              ? PlyElement(elementKey,
                           (int)source.size() / (int)propertyKeys.size())
              : PlyElement(elementKey, (int)source.size() / listCount);
 ...
 private:
  ...
  void read_property_ascii(PlyProperty::Type t, void *dest,
                           uint32_t &destOffset, std::istream &is);
  void write_property_ascii(PlyProperty::Type t, std::ostringstream &os,
                            uint8_t *src, uint32_t &srcOffset);
  void write_property_binary(PlyProperty::Type t, std::ostringstream &os,
                             uint8_t *src, uint32_t &srcOffset);
